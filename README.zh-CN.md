...
> **三条路，你把门。** Claude Code 的规格驱动开发流水线。
...
| 命令 | 场景 | 流水线 |
|---|---|---|
| `/trivium:feature` | 新增需求 | 存量代码考古 → 苏格拉底消歧 → PRD → 架构与冻结 API 合同（复用优先 ADR + 迁移/回滚方案）→ UI 设计（设计系统出屏，视觉签核）→ 任务计划 → subagent 实现（TDD + 两阶段 review）→ 全量回归 + Playwright 验收 |
...
**UI 门**（feature Gate 3）是设计 MCP（如 Stitch）用项目设计系统把真实界面渲染出来、由你在**任何页面被实现之前**签核外观的地方——从而让各 feature 的界面保持是同一个产品，而不是各自漂移。

## Trivium 在 superpowers 之上补了什么

Trivium **编排** [superpowers](https://github.com/obra/superpowers) 而非重新实现它。superpowers 提供方法论（brainstorming、writing-plans、subagent 开发、TDD、系统化调试、code review）；Trivium 补齐规格驱动流水线需要而 superpowers 没有的几块：
...
- **`design-system`** — 项目级**视觉**语言（色彩、字体、间距、圆角、组件外观、动效性格），在 inception 冻结、由每个 feature 的界面**组合复用而非各自发明**，使不同 subagent 做出的 UI 看起来是同一个产品。严格与业务无关：外观住这里，界面住各 feature。它是 `api-contract` 的视觉对偶。
...
Trivium 的结构探查环节——代码考古、影响 / 爆炸半径分析、根因链路追踪——在检测到**代码智能 MCP 或 LSP**时会自动优先使用图查询与调用链追踪，而非文本搜索，来回答"谁调用了它""改动会波及什么"。这是**能力探测、完全可选**的：没有此类工具时，Trivium 退回 subagent 探索。（唯一硬依赖是 [superpowers](https://github.com/obra/superpowers)——它是方法论，不是工具。）

- **GitNexus** —— 代码图谱 MCP，提供影响 / 爆炸半径查询，并带一个 PostToolUse hook 在每次提交后自动重建索引，保证爆炸半径结果新鲜。其开源版采用 **PolyForm Noncommercial** 许可；商用需走企业授权——用于营利场景前请先确认许可适用。

依赖代码图谱工具时有两条铁律：查询前确认**索引新鲜**（陈旧图谱比没有更危险），并核实工具**许可**覆盖你的使用场景。
