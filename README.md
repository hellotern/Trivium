...
|---|---|---|
| `/trivium:feature` | New requirements | brownfield codebase archaeology → disambiguation → PRD → architecture & frozen API contract (reuse-first ADRs + migration/rollback plan) → task plan → subagent implementation (TDD + two-stage review) → full regression + Playwright acceptance |
